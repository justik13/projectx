...
class TestSecurityExtended:
    ...
    def test_is_safe_url_valid_http(self):
        ...
        assert is_safe_url("https://api.example.com/v1/endpoint") is True

    @pytest.mark.unit
    def test_is_safe_url_ftp_allowed(self):
        """FTP с публичным hostname разрешён (функция не проверяет scheme)"""
        from utils.security import is_safe_url

        # FTP с валидным hostname разрешён
        assert is_safe_url("ftp://example.com") is True
        assert is_safe_url("ftp://ftp.example.org") is True

    @pytest.mark.unit
    def test_is_safe_url_no_hostname_blocked(self):
        """URLs без hostname блокируются (это правильное поведение безопасности)"""
        from utils.security import is_safe_url

        # file:// и javascript: имеют пустой hostname - блокируются
        assert is_safe_url("file:///etc/passwd") is False  # Нет hostname
        assert is_safe_url("javascript:alert(1)") is False  # Нет hostname
        assert is_safe_url("data:text/html,<h1>hi</h1>") is False  # Нет hostname

    @pytest.mark.unit
    def test_is_safe_url_empty_hostname(self):
        from utils.security import is_safe_url
    ...
    @pytest.mark.unit
    def test_is_safe_url_private_ip_ranges(self):
        from utils.security import is_safe_url
    ...
    @pytest.mark.unit
    def test_is_safe_url_metadata_endpoints(self):
        from utils.security import is_safe_url
    ...
    @pytest.mark.unit
    def test_is_safe_url_localhost_allowed(self):
        from utils.security import is_safe_url

        assert is_safe_url("http://localhost") is True
        assert is_safe_url("http://localhost:8080") is True
        assert is_safe_url("http://127.0.0.1") is True
        assert is_safe_url("http://127.0.0.1:4001") is True
        assert is_safe_url("http://[::1]") is True
        assert is_safe_url("http://0.0.0.0") is True

    @pytest.mark.unit
    def test_is_safe_url_domain_resolves_to_private(self):
        from utils.security import is_safe_url
        ...
        with patch('utils.security.socket.getaddrinfo') as mock_getaddrinfo:
            ...
            assert is_safe_url("http://internal.company.local") is False

    @pytest.mark.unit
    def test_is_safe_url_domain_resolves_to_public(self):
        from utils.security import is_safe_url
    ...
    @pytest.mark.unit
    def test_is_safe_url_dns_resolution_fails(self):
        from utils.security import is_safe_url
        import socket

        with patch('utils.security.socket.getaddrinfo', side_effect=socket.gaierror):
            assert is_safe_url("http://nonexistent.domain.xyz") is True

    @pytest.mark.unit
    def test_is_safe_url_ipv6_addresses(self):
        from utils.security import is_safe_url
    ...
    @pytest.mark.unit
    def test_is_safe_url_exception_handling(self):
        from utils.security import is_safe_url
