...
async def soft_delete_user(
    ...
    Раньше: CASCADE-удаление → VPN-профили удалялись из БД,
    но клиенты на серверах Amnezia оставались (если API недоступен).
    Worker cleanup подчищал через 24ч, но с задержкой.
    
    Теперь: Soft delete — помечаем is_deleted=True,
    VPN-профили остаются в БД для фоновой задачи удаления из API.
    После успешного удаления из API — фактическое удаление из БД.
    
    Returns:
        bool: True если успешно помечен как удалённый
    """
    if user.is_deleted:
...
async def get_soft_deleted_users_with_profiles(
    session: AsyncSession, older_than_hours: int = 24
) -> list[User]:
    """
    Возвращает пользователей с is_deleted=True и их VPN-профилями.
    
    Используется фоновой задачей для очистки API.
    Фильтр по времени: только те, кто был удалён более N часов назад.
    Это даёт время для восстановления (edge case) перед удалением из API.
    """
    ...
    stmt = (
        ...
        .options(selectinload(User.profiles).selectinload(User.profiles))  # profiles уже загружены
    )
    result = await session.execute(stmt)
    return result.scalars().unique().all()