...
class PlategaClient:
    ...
    async def create_transaction(
        ...
        """
        Создание платежной ссылки с заданным методом (СБП QR)
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(url, json=data, headers=self._get_headers(), timeout=30) as response:
