...
class ReferralService:
    ...
    async def process_bonus(
        ...
        *,
        ...
        if not user:
            logger.warning(
                f"Referral bonus: user {user_telegram_id} not found in DB"
            )
        ...
        if user.is_deleted or user.is_banned:
            logger.info(
                f"Referral bonus SKIPPED: user {user_telegram_id} "
                f"is deleted or banned"
            )
        ...
        if referrer.is_deleted or referrer.is_banned:
            logger.info(
                f"Referral bonus SKIPPED: referrer {referrer_telegram_id} "
                f"is deleted or banned"
            )
