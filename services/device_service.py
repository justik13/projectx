...
# 🔥 ИСПРАВЛЕНО: Кастомные исключения вместо динамических атрибутов
...
class DeviceService:
    ...
    async def create_device(
        ...
        Raises:
            ServerUnavailable: Сервер не найден, отключен или переполнен.
            DailyLimitExceeded: Превышен дневной лимит создания устройств.
            InvalidConfig: API вернул невалидный конфиг.
            DeviceLimitExceeded: Достигнут лимит устройств по тарифу.
            IntegrityError: Ошибка БД (дубликат).
        """
        server = await get_server_by_id(session, server_id)
        if not server or server.protocol != AMNEZIA_PROTOCOL:
            logger.warning(f"create_device: invalid server {server_id} or protocol mismatch")
        ...
        async with lock:
            ...
            if not is_admin(user.telegram_id):
                ...
                if user.device_creations_today >= DEVICE_DAILY_LIMIT:
                    logger.warning(f"create_device: user {user.telegram_id} exceeded daily limit")
                    try:
            ...
            if not is_valid_vpn_uri(raw_config):
                logger.error(f"create_device: API returned invalid vpn:// URI. Rolling back.")
                try:
                ...
                async with session.begin_nested() as savepoint:
                    ...
                    if profiles_count >= user.device_limit:
                        ...
                        try:
                            ...
                            logger.error(f"Failed to rollback API client after limit check: {rollback_error}")
    ...
    async def delete_device(session: AsyncSession, profile: VPNProfile) -> bool:
        ...
        if not deleted:
            logger.error(f"delete_device: API delete_user failed for peer_id={profile.peer_id[:16]}...")
            return False
        ...
        try:
            async with session.begin_nested() as savepoint:
                ...
                try:
                    await AuditService.log_action(
                        session, admin_id=profile.user.telegram_id if hasattr(profile, 'user') else 0,
