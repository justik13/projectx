...
# Per-user locks для защиты от race condition внутри одного пользователя
...
class DeviceService:
    ...
    async def create_device(
        ...
        # ═══════════════════════════════════════════════════════════
        # 🔥 ИСПРАВЛЕНО: Убран server_lock (MUTEX bottleneck)
        #
        # Защита от overselling обеспечивается тремя уровнями:
        # 1. Read-only проверка слотов из БД (O(1) COUNT)
        # 2. get_real_peer_count() через slots_cache (имеет свой per-server lock)
        # 3. Amnezia API вернёт ошибку если слотов нет → IntegrityError → rollback
        #
        # Было: server_lock = MUTEX → 50 concurrent × 600ms = 30 секунд очередь
        # Стало: параллельное выполнение → все 50 за ~600ms
        # ═══════════════════════════════════════════════════════════
        ...
        # Шаг 2: Если свободно менее CRITICAL_SLOTS_THRESHOLD — идём в API
