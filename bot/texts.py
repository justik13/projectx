"""
🔒 ТЕКСТЫ БОТА — ПРОВЕРКА НА ЗАПРЕЩЁННЫЕ СЛОВА
═══════════════════════════════════════════════
ЖЁСТКОЕ ПРАВИЛО #5: Запрещённые слова
- VPN ❌
- впн ❌
- обход блокировок ❌
- глушилки ❌
...
Использованные замены:
- "защищённые протоколы" вместо "шифрования" (FAQ, строка ~150)
- "конфигурации сети" вместо "VPN" (платёжные тексты)
- "доступ" вместо "VPN" (уведомления)
- "ProjectX" вместо "VPN-сервис" (брендинг)
...
🔥 ИСПРАВЛЕНО:
- Слово "шифрования" заменено на "защищённые протоколы" (FAQ)
- Слово "VPN" НЕ используется в пользовательских текстах
- Только ProjectX как бренд

Форматирование: ТОЛЬКО HTML (<b>, <i>, <code>, <pre>)
Markdown ЗАПРЕЩЁН ( _, *, ` )
"""

# ═══════════════════════════════════════════════════════════════
# 🔥 ПОДГОТОВКА К i18n (Проблема #22)
# ═══════════════════════════════════════════════════════════════
# В будущем все тексты будут вынесены в JSON/YAML файлы с локалями:
# locales/
#   ru.json
#   en.json
# Структура:
...
#   "hub_header": "Главное меню",
#   ...
# }
# Использование:
# from bot.i18n import get_text
# text = get_text("welcome_text", locale="ru")
#
# Пока все тексты hardcoded на русском (acceptable для стадии тестирования).
# Миграция на i18n будет выполнена при появлении первых нерусскоязычных пользователей.
# ═══════════════════════════════════════════════════════════════
