...
from utils.telegram import safe, render_hub, clear_hub_cache
...
async def cmd_start(message: Message, state: FSMContext, command: Command, session: AsyncSession):
    ...
    if invoice_message_id:
        ...
        try: await message.bot.delete_message(message.chat.id, invoice_message_id)
        except Exception as e: logger.warning(f"Failed to delete invoice message: {e}")
    if cancel_message_id:
        try: await message.bot.delete_message(message.chat.id, cancel_message_id)
        except Exception as e: logger.warning(f"Failed to delete cancel message: {e}")
    if payment_id:
        try:
            await mark_payment_as_cancelled(session, payment_id)
            logger.info(f"Payment {payment_id} cancelled due to /start")
        except Exception as e: logger.warning(f"Failed to cancel payment {payment_id}: {e}")
...
async def back_to_main_menu(callback: CallbackQuery, state: FSMContext, db_user: User | None = None, session: AsyncSession = None):
    ...
    if invoice_message_id:
        clear_hub_cache(callback.message.chat.id)
        try: await callback.bot.delete_message(callback.message.chat.id, invoice_message_id)
        except Exception: pass
    if cancel_message_id:
        try: await callback.bot.delete_message(callback.message.chat.id, cancel_message_id)
        except Exception: pass
    if payment_id:
        try: await mark_payment_as_cancelled(session, payment_id)
        except Exception: pass
