...
async def cmd_start(message: Message, state: FSMContext, command: Command, session: AsyncSession):
    ...
    # Отменяем платёж в БД (если был незавершён)
    if payment_id:
        try:
    ...
    # render_hub теперь делает send-first: отправляет новое, потом удаляет старое
    # Это устраняет пустой экран — пользователь сразу видит новое сообщение
