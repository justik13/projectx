...
from database.repositories.tariffs_repo import (
    create_tariff,
    get_all_tariffs,        # noqa: F401 — для обратной совместимости
    get_tariff_by_id,
    get_tariff_count,
...
async def _build_tariffs_list_text_and_kb(
    ...
    if not tariffs:
        ...
        for tariff in tariffs:
            ...
            builder.button(
                text=f"{status} {tariff.duration_days} дн. · {tariff.price_rub}₽ / {tariff.price_stars}⭐",
...
async def process_add_tariff(message: Message, state: FSMContext, session: AsyncSession):
    ...
    if step == "days":
        ...
        await message.answer(
            texts.ADMIN_TARIFF_RUB_PROMPT.format(days=days),
            reply_markup=get_back_button("admin_tariffs"),
        )
