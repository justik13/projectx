...
async def process_add_server(
    ...
    if step == "name":
        ...
        await state.update_data(
            country_flag=message.text.strip(), step="api_url"
        )
        await render_hub(
            message.bot, message.chat.id,
        ...
        await AuditService.log_action(
            session, message.from_user.id, "ADD_SERVER",
            "Server", server.id, api_server_name,
        )
...
async def confirm_delete_server(
    ...
    result = await session.execute(
        select(VPNProfile.id, VPNProfile.peer_id).where(
