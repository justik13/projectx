...
async def _show_user_card_edit(message, user, session: AsyncSession):
    ...
    try:
        await message.edit_text(
            rendered, reply_markup=get_admin_user_card_keyboard(user.telegram_id), parse_mode="HTML",
        )
    except Exception:
        await render_hub(message.bot, message.chat.id, rendered, get_admin_user_card_keyboard(user.telegram_id))
...
async def show_extend_options(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        await callback.message.edit_text(
            texts.ADMIN_EXTEND_HEADER.format(telegram_id=telegram_id),
...
async def extend_subscription(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    if not is_admin(callback.from_user.id):
        ...
        return
    await state.clear()
    _, telegram_id_str, days_str = callback.data.split(":")
...
async def start_custom_extend(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        await callback.message.edit_text(
            texts.ADMIN_CUSTOM_EXTEND_HEADER.format(telegram_id=telegram_id),
...
async def process_custom_days(message: Message, state: FSMContext, session: AsyncSession):
    if not is_admin(message.from_user.id):
        ...
        return
    if not message.text:
        await render_hub(message.bot, message.chat.id, texts.ERROR_DAYS_RANGE.format(min=1, max=36500), get_back_button("admin_menu"))
        return
    if message.text.startswith("/"):
        await state.clear()
        return
    ...
    await render_hub(
        message.bot, message.chat.id,
        texts.ADMIN_EXTENDED_CUSTOM.format(
            telegram_id=telegram_id, days=days,
            valid_until=format_datetime(user.subscription_end),
        ),
        get_admin_user_card_keyboard(telegram_id), parse_mode="HTML",
    )
    await state.clear()
...
async def show_user_devices(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    if not user:
        ...
        return
    profiles = await get_user_profiles(session, user.id)
    rendered = texts.ADMIN_USER_DEVICES_HEADER.format(telegram_id=telegram_id)
    if not profiles:
        rendered += texts.ADMIN_USER_DEVICES_EMPTY
    else:
        for p in profiles:
            rendered += (
