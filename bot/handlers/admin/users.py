...
from utils.telegram import safe, safe_edit_text
...
async def _build_users_list_text_and_kb(
    ...
    if not users:
        ...
        for user in users:
            ...
            username = (
                f"@{safe(user.username)}" if user.username else f"ID:{user.telegram_id}"
            )
...
async def process_search_user(message: Message, state: FSMContext, session: AsyncSession):
    if not is_admin(message.from_user.id):
        ...
        return
    if not message.text:
        await message.answer(texts.ERROR_NUMERIC_ID)
        return
...
async def process_custom_days(message: Message, state: FSMContext, session: AsyncSession):
    if not is_admin(message.from_user.id):
        ...
        return
    if not message.text:
        await message.answer(texts.ERROR_DAYS_RANGE.format(min=1, max=36500))
        return
    if message.text.startswith("/"):
        await state.clear()
        return
