...
async def _render_dashboard(
    ...
    if maintenance_enabled:
        rendered += texts.DASHBOARD_MAINTENANCE_ON
    else:
        rendered += texts.DASHBOARD_MAINTENANCE_OFF
    ...
    try:
        ...
        logger.debug("_render_dashboard render_hub failed: %s", e)
...
async def admin_maintenance_menu(
    ...
    if maintenance_enabled:
        text = texts.ADMIN_MAINTENANCE_MENU_ENABLED
    else:
        text = texts.ADMIN_MAINTENANCE_MENU_DISABLED
...
async def admin_maintenance_toggle_apply(
    ...
    try:
        ...
        if new_status:
            ...
            await callback.answer(
                texts.ADMIN_MAINTENANCE_DISABLED_ANSWER,
                show_alert=True,
            )
        ...
        await callback.answer(
            texts.ADMIN_MAINTENANCE_TOGGLE_FAILED,
            show_alert=True,
        )
        return
