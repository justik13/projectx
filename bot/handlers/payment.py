...
from bot.keyboards import (get_back_button, get_payment_method_keyboard, get_tariff_showcase_keyboard, get_tariff_duration_keyboard, get_renew_keyboard, get_change_tariff_keyboard, get_payment_success_keyboard)
...
async def hub_menu_payment(callback: CallbackQuery, state: FSMContext, db_user: User | None = None, session: AsyncSession = None):
    await state.clear()
    if not db_user:
...
async def _show_showcase(target, session: AsyncSession):
    ...
    if not tariffs:
        await target.edit_text(texts.PAYMENT_NO_TARIFFS, reply_markup=get_back_button())
        return
    grouped = {}
    for t in tariffs:
    ...
    try: await target.edit_text(texts.PAYMENT_SHOWCASE_HEADER, reply_markup=kb, parse_mode="HTML")
    except TelegramBadRequest: await target.answer(texts.PAYMENT_SHOWCASE_HEADER, reply_markup=kb, parse_mode="HTML")

async def _show_hub(target, user: User, session: AsyncSession):
    profiles = await get_user_profiles(session, user.id)
    tariff_name = _get_tariff_display_name(user.device_limit)
    text = texts.PAYMENT_HUB_HEADER.format(valid_until=format_datetime(user.subscription_end), days_left=format_days_left(user.subscription_end), tariff_name=tariff_name, devices_count=len(profiles), device_limit=user.device_limit)
    ...
    builder.adjust(1)
    try: await target.edit_text(text, reply_markup=builder.as_markup(), parse_mode="HTML")
    except TelegramBadRequest: await target.answer(text, reply_markup=builder.as_markup(), parse_mode="HTML")
...
async def show_quick_renew(callback: CallbackQuery, db_user: User, session: AsyncSession):
    ...
    text = texts.PAYMENT_QUICK_RENEW_HEADER.format(tariff_name=tariff_name, valid_until=format_datetime(db_user.subscription_end))
...
async def show_change_tariff(callback: CallbackQuery, db_user: User, session: AsyncSession):
    ...
    current_limit = db_user.device_limit
    tariff_name = _get_tariff_display_name(current_limit)
    text = texts.PAYMENT_CHANGE_TARIFF_HEADER.format(tariff_name=tariff_name, valid_until=format_datetime(db_user.subscription_end))
    kb = get_change_tariff_keyboard(tariffs, current_limit)
    await callback.message.edit_text(text, reply_markup=kb, parse_mode="HTML")

@router.callback_query(F.data.startswith("select_tariff:"))
async def select_tariff(callback: CallbackQuery, state: FSMContext, db_user: User | None = None, session: AsyncSession = None):
    ...
    if db_user and await _is_subscription_active(db_user):
        ...
        if device_limit < current_limit:
            text = texts.PAYMENT_DOWNGRADE_BLOCKED.format(current_limit=current_limit, new_limit=device_limit, valid_until=format_datetime(db_user.subscription_end))
    ...
    text = texts.PAYMENT_CHECKOUT_TEXT.format(tariff_name=tariff_name, duration_days=tariff.duration_days, price_rub=tariff.price_rub, price_stars=tariff.price_stars)
...
async def pay_stars(callback: CallbackQuery, state: FSMContext, db_user: User | None = None, session: AsyncSession = None):
    ...
    try:
        await callback.bot.send_invoice(chat_id=callback.from_user.id, title=f"Подписка на {tariff.duration_days} дней ({getattr(tariff, 'device_limit', 2)} устр.)", description="Оплата цифрового доступа к защищенным конфигурациям сети.", prices=[LabeledPrice(label="Доступ к сети", amount=tariff.price_stars)], provider_token="", payload=f"stars_payment:{payment.id}", currency="XTR", start_parameter="network-access-stars")
...
async def process_successful_payment(message: Message, state: FSMContext, session: AsyncSession = None):
    ...
    if await PaymentService.handle_successful_payment(session, payment_id):
        ...
        tariff_name = _get_tariff_display_name(device_limit)
        text = texts.PAYMENT_SUCCESS_RENEW.format(tariff_name=tariff_name, valid_until=valid_until) if profiles else texts.PAYMENT_SUCCESS_NEW.format(tariff_name=tariff_name, valid_until=valid_until)
...
async def pay_sbp(callback: CallbackQuery, state: FSMContext, session: AsyncSession = None):
    ...
    builder.adjust(1)
    await callback.message.edit_text(texts.PAYMENT_SBP_TEXT.format(price_rub=tariff.price_rub), reply_markup=builder.as_markup(), parse_mode="HTML")
...
async def back_to_payment(callback: CallbackQuery, state: FSMContext, session: AsyncSession = None):
    ...
    tariff_name = _get_tariff_display_name(device_limit)
    text = texts.PAYMENT_CHECKOUT_TEXT.format(tariff_name=tariff_name, duration_days=tariff.duration_days, price_rub=tariff.price_rub, price_stars=tariff.price_stars)
    try: await callback.message.edit_text(text, reply_markup=get_payment_method_keyboard(tariff.id), parse_mode="HTML")
