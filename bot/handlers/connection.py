...
async def download_conf(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if parsed is None or not is_valid_amneziawg_config(parsed):
            logger.warning(f"download_conf: failed to parse vpn:// URI for profile {profile_id}")
            await callback.message.answer(
...
async def start_add_device(callback: CallbackQuery, state: FSMContext):
    ...
    session = await get_session()
    try:
        # 🔥 ИСПРАВЛЕНИЕ: Используем get_available_servers вместо get_active_servers
        servers = await get_available_servers(session)
        if not servers:
            await callback.answer("❌ На всех серверах закончились свободные слоты. Попробуйте позже или напишите в поддержку.", show_alert=True)
            return
