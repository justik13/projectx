...
async def _get_effective_device_limit(user: User, session: AsyncSession) -> int:
    if user.current_tariff_id:
        ...
        if tariff: return tariff.device_limit
    return 0
...
async def _render_connections(target, user: User, session: AsyncSession):
    if not user:
        await render_hub(target.bot, target.chat.id, texts.ERROR_USER_NOT_FOUND, get_back_button("back_to_main_menu"))
        return
...
async def show_config(callback: CallbackQuery, state: FSMContext, session: AsyncSession, db_user: User | None = None):
    ...
    profile_id = int(callback.data.split(":")[1])
    profile = await get_profile_by_id(session, profile_id)
    if not profile or not db_user or profile.user_id != db_user.id: return
...
async def download_conf(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    if conf_content is None:
        await render_hub(callback.bot, callback.message.chat.id, "⚠️ Ошибка сборки .conf", get_back_button(f"manage_device:{profile.id}"))
        return
    ...
    # Удаляем текстовый хаб и отправляем документ как единственное сообщение
...
async def rename_device_start(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    await render_hub(callback.bot, callback.message.chat.id, texts.DEVICE_RENAME_PROMPT, get_back_button(f"manage_device:{profile_id}"))
...
async def rename_device_process(message: Message, state: FSMContext, session: AsyncSession):
    # CleanChatMiddleware удалил сообщение пользователя
    ...
    if not new_name or len(new_name) > 16 or not DEVICE_NAME_REGEX.match(new_name):
        await render_hub(message.bot, message.chat.id, texts.ERROR_INVALID_DEVICE_NAME, get_back_button("back_to_connections"))
        return
    ...
    if profile:
        ...
        await render_hub(message.bot, message.chat.id, f"✅ Устройство переименовано в <b>{safe(new_name)}</b>", get_device_keyboard(profile.id))
    await state.clear()
...
async def request_delete_device(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    profile = await get_profile_by_id(session, profile_id)
    if not profile: return
    await render_hub(callback.bot, callback.message.chat.id, texts.DEVICE_DELETE_CONFIRM.format(device_name=safe(profile.device_name)), get_device_delete_confirm_keyboard(profile_id))
...
async def cancel_delete_device(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    await state.clear()
    profile_id = int(callback.data.split(":")[1])
    await render_hub(callback.bot, callback.message.chat.id, f"📱 <b>Управление устройством</b>", get_device_keyboard(profile_id))

@router.callback_query(F.data.startswith("confirm_delete_device:"))
async def confirm_delete_device(callback: CallbackQuery, state: FSMContext, session: AsyncSession, db_user: User | None = None):
    await callback.answer("⏳ Удаляю устройство...")
    await state.clear()
    profile_id = int(callback.data.split(":")[1])
    profile = await get_profile_by_id(session, profile_id)
    if not profile: return
    if not await DeviceService.delete_device(session, profile): return
    user = db_user or await get_user_by_telegram_id(session, callback.from_user.id)
    if user: await _render_connections(callback.message, user, session)
...
async def start_add_device(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    await render_hub(callback.bot, callback.message.chat.id, texts.CONNECTION_SELECT_SERVER, builder.as_markup())
...
async def enter_device_name(message: Message, state: FSMContext, session: AsyncSession, db_user: User | None = None):
    ...
    if not device_name or len(device_name) > 16 or not DEVICE_NAME_REGEX.match(device_name):
        await render_hub(message.bot, message.chat.id, texts.ERROR_INVALID_DEVICE_NAME, get_back_button("add_device"))
        return
    ...
    if profiles_count >= device_limit:
        await render_hub(message.bot, message.chat.id, texts.ERROR_DEVICE_LIMIT_REACHED.format(limit=device_limit), get_back_button("back_to_connections"))
        await state.clear()
        return
    ...
    if not profile:
        await render_hub(message.bot, message.chat.id, texts.ERROR_SERVER_UNAVAILABLE, get_back_button("back_to_connections"))
        await state.clear()
        return
        
    server = await get_server_by_id(session, profile.server_id)
    success_text = texts.DEVICE_ADDED_SUCCESS.format(device_name=safe(device_name), flag=server.country_flag, server_name=safe(server.name))
