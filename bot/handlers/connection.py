...
async def _build_connections_screen(
    ...
    rendered = texts.CONNECTION_LIST_HEADER.format(
        count=profiles_count, limit=user.device_limit,
    )
...
async def _render_connections(
    target, user: User, session: AsyncSession, *, edit: bool,
):
    if not user:
        if edit:
            await target.edit_text(texts.ERROR_USER_NOT_FOUND)
        else:
            await target.answer(texts.ERROR_USER_NOT_FOUND)
        return

    if not await SubscriptionService.check_access(session, user.telegram_id):
        if edit:
            await target.edit_text(texts.ERROR_NO_SUBSCRIPTION, parse_mode="HTML")
        else:
            await target.answer(texts.ERROR_NO_SUBSCRIPTION, parse_mode="HTML")
        return
...
async def show_connections(
    message: Message, state: FSMContext, session: AsyncSession, db_user: User | None = None,
):
    await state.clear()
    await safe_delete_message(message)
    await _render_connections(message, db_user, session, edit=False)


@router.callback_query(F.data == "back_to_connections")
async def back_to_connections(
    callback: CallbackQuery, state: FSMContext, session: AsyncSession, db_user: User | None = None,
):
    await state.clear()
    if not db_user:
...
# ============================================================
# Управление конкретным устройством
# ============================================================
...
async def manage_device(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    rendered = texts.DEVICE_MANAGE_HEADER.format(
        ...
        traffic_total=format_traffic(profile.traffic_down + profile.traffic_up),
        last_connected=(
            format_datetime(profile.last_connected) if profile.last_connected else "Нет данных"
        ),
    )

    await callback.message.edit_text(
        rendered, reply_markup=get_device_keyboard(profile.id), parse_mode="HTML",
    )
    await callback.answer()
...
async def download_conf(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    await callback.bot.send_chat_action(
        chat_id=callback.from_user.id, action="upload_document",
    )

    safe_device_name = (
        "".join(c for c in profile.device_name if c.isalnum() or c in (" ", "_", "-")).strip()
        or "client"
    )
...
async def rename_device_process(message: Message, state: FSMContext, session: AsyncSession):
    ...
    if message.text.startswith("/") or message.text in REPLY_MENU_BUTTONS:
        ...
        await message.answer(
            ...
            reply_markup=get_back_button("back_to_connections"),
        )
        return
...
async def enter_device_name(
    ...
    if await get_user_profiles_count(session, user.id) >= user.device_limit:
        await message.answer(
            texts.ERROR_DEVICE_LIMIT_REACHED.format(limit=user.device_limit),
            parse_mode="HTML",
        )
        await state.clear()
        return
    ...
    await message.answer(
        texts.DEVICE_ADDED_SUCCESS.format(
